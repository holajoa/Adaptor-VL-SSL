...
class MGCA(LightningModule):
    ...
    def forward(self, batch, batch_idx, split="train"):
        ...
        word_sim = (
            torch.bmm(word_emb_q, word_atten_output.permute(0, 2, 1))
        ...
        targets = torch.arange(word_num).type_as(word_emb_q).long().repeat(bz)
        loss_word_1 = (
            torch.sum(
                F.cross_entropy(word_sim_1, targets, reduction="none")
                * word_atten_weights.view(-1)
            )
            / bz
        )
        ...
        loss_word_2 = (
            torch.sum(
                F.cross_entropy(word_sim_2, targets, reduction="none")
                * word_atten_weights.view(-1)
            )
            / bz
        )
        ...
        if self.hparams.bidirectional:
            ...
            if "vit" not in self.hparams.img_encoder:
                ...
                patch_atten_weights = (
                    torch.ones(bz, patch_num).type_as(batch["imgs"]) / patch_num
                )

            else:
                with torch.no_grad():
                    img_attn_map = self.img_encoder_q.model.blocks[
                        -1
                    ].attn.attention_map.detach()
                    atten_weights = img_attn_map[:, :, 0, 1:].mean(dim=1)
                    patch_atten_weights = []
                    for i in range(bz):
                        atten_weight = atten_weights[i]
                        atten_weight = atten_weight.clip(
                            torch.quantile(atten_weight, 0.1),
                            torch.quantile(atten_weight, 0.9),
                        )
            ...
            patch_sim = (
                torch.bmm(patch_emb_q, patch_atten_output.permute(0, 2, 1))
                / self.hparams.local_temperature
            )
            ...
            loss_patch_1 = (
                torch.sum(
                    F.cross_entropy(patch_sim_1, targets, reduction="none")
                    * patch_atten_weights.view(-1)
                )
                / bz
            )
            ...
            loss_patch_2 = (
                torch.sum(
                    F.cross_entropy(patch_sim_2, targets, reduction="none")
                    * patch_atten_weights.view(-1)
                )
                / bz
            )

            loss_patch = (loss_patch_1 + loss_patch_2) / 2.0
        ...
        with torch.no_grad():
            img_code = torch.exp(img_proto_out / self.hparams.epsilon).t()
            img_code = self.get_assignments(
                img_code, self.hparams.sinkhorn_iterations
            )  # bz, 500
            report_code = torch.exp(report_proto_out / self.hparams.epsilon).t()
            report_code = self.get_assignments(
                report_code, self.hparams.sinkhorn_iterations
            )  # bz, 500
        ...
        loss_proto = (loss_i2t_proto + loss_t2i_proto) / 2.0
    ...
    def training_step(self, batch, batch_idx):
        loss_ita, loss_local, loss_proto, acc1, acc5 = self(batch, batch_idx, "train")
        loss = (
        ...
        self.log_dict(
            log, batch_size=self.hparams.batch_size, sync_dist=True, prog_bar=True
        )
    ...
    def validation_step(self, batch, batch_idx):
        loss_ita, loss_local, loss_proto, acc1, acc5 = self(batch, batch_idx, "valid")
...
def cli_main():
    ...
    ckpt_dir = os.path.join(BASE_DIR, f"../../../data/ckpts/MGCA/{extension}")
