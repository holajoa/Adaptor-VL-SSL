...
class AdaptorPipelineBase(LightningModule):
    def __init__(
        ...
        self.text_model = BertModel.from_pretrained(
            self.text_pretrained, cache_dir=CACHE_DIR
        )
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.vision_model_type in "timm":
            ...
            image_embeds_raw = torch.flatten(vision_outputs, start_dim=2).permute(
                (0, 2, 1)
            )  # .mean(1)
...
class AdaptorPipelineWithClassificationHead(AdaptorPipelineBase):
    def __init__(
        ...
        if self.num_classes == 1:
            ...
            self.loss_func = nn.CrossEntropyLoss(
                weight=torch.ones(num_classes), reduction="mean"
            )
        if self.num_classes > 1:
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log(
            f"train_{self.metric_name}_step", self.metrics, on_step=True, prog_bar=True
        )
    ...
    def test_epoch_end(self, outputs):
        self.log(
            f"test_{self.metric_name}_epoch",
