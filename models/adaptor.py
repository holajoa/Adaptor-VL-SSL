...
class TransformerEncoderLayerWithCrossAttention(nn.Module):
    ...
    def forward(self, src, query=None, src_mask=None, src_key_padding_mask=None, run_cross_attn=False):
            ...
            if run_cross_attn:
                ...
                src2 = self.attn(query, src, src, attn_mask=src_mask,
    ...
    def forward(
        ...
        **kwargs,
        ...
        if text_embeds_raw is not None:
            image_embeds_full, text_embeds_full = self.projection(image_embeds_raw, text_embeds_raw)
            image_embeds_full, text_embeds_full = self.adaptor_module(image_embeds_full, text_embeds_full)  # bz x seq_len x 768 during pretraining
            ...
            image_embeds, text_embeds = image_embeds_full[:, 0, :], text_embeds_full[:, 0, :]

            # normalized features - is this necessary for full embeddings?
            image_embeds_full = image_embeds_full / image_embeds_full.norm(dim=-1, keepdim=True)
            text_embeds_full = text_embeds_full / text_embeds_full.norm(dim=-1, keepdim=True)
