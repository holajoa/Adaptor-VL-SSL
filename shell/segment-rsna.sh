...
export DATASET="rsna"
export PROJECT_POSTFIX="_batchsize16"
for DATA_PCT in 1.0
do
    for TEXT_MODEL in "bert" "biobert" "clinicalbert" "pubmedbert" "cxrbert"
    do
        for VISION_MODEL in "resnet-ae" "dinov2-b" "dinov2-s" 
        do
            echo ${VISION_MODEL}_${TEXT_MODEL}_${DATASET}_${DATA_PCT}
            python ./segment.py --dataset $DATASET --vision_model $VISION_MODEL --text_model $TEXT_MODEL --batch_size 6 --data_pct $DATA_PCT --num_workers 1 --num_train_epochs 100 --seed 1117 --lr 5e-4 --weight_decay 1e-4 --output_dir $SAVED_MODEL_DIR/${VISION_MODEL}_${TEXT_MODEL}_${DATASET}_${DATA_PCT} --postfix v2 --pretrain_wandb_project_name adaptor_pretrain_2_layers --wandb --project_name adaptor_segment_2_layers${PROJECT_POSTFIX} --check_val_every_n_epochs 1 --patience_epochs 5
