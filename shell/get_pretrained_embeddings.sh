...
python3 -m torch.distributed.launch --nproc_per_node 2 ./get_pretrained_embeddings.py --vision_model swin-base --batch_size 64 --data_pct 1.0 --seed 42  --num_workers 8 --force_rebuild_dataset
# python3 -m torch.distributed.launch --nproc_per_node 2 ./get_pretrained_embeddings.py --text_model biobert --batch_size 64 --data_pct 1.0 --seed 42  --num_workers 8 --force_rebuild_dataset
